...
ax.get_figure().savefig(pj(scripts_dir,'24-7','violin_swarm_gender_site_1_8.png'))
#%%
ax = sns.violinplot(x="site", y="age", hue="gender",
                    data=id_features.loc[id_features["site"]>8], palette="muted", split=True, inner = None)
ax = sns.swarmplot(x="site", y="age", data=id_features.loc[id_features["site"]>8],
                   color="black", size=3)
ax.get_figure().savefig(pj(scripts_dir,'24-7','violin_swarm_gender_site_9_16.png'))

#%%
ax = sns.violinplot(x="site", y="age", hue="gender",
                    data=id_features, palette="muted", split=True)
ax.get_figure().savefig(pj(scripts_dir,'24-7','violin_gender_site.png'))
